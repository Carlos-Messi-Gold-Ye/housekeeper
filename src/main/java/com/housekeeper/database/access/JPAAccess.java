...
public class JPAAccess {
    ...
    public <T> List<T> findAll(Class<T> entityClass) {
        ...
        try {
            ...
            logger.debug("find all, entityClass={}, elapsedTime={}", entityClass.getName(), watch.elapsedTime());
        }
    }
    ...
    public <T> List<T> find(CriteriaQuery<T> query) {
        ...
        try {
            ...
            nativeSql = typedQuery.unwrap(org.hibernate.Query.class).getQueryString();
            return typedQuery.getResultList();
        } finally {
            logger.debug("find by CriteriaQuery<T>, elapsedTime={}, nativeSql={}", watch.elapsedTime(), nativeSql);
        }
    }
    ...
    public <T> List<T> find(CriteriaQuery<T> query, int offset, int fetchSize) {
        ...
        try {
            TypedQuery<T> typedQuery = entityManager.createQuery(query);
            nativeSql = typedQuery.unwrap(org.hibernate.Query.class).getQueryString();
            return typedQuery.setFirstResult(offset).setMaxResults(fetchSize).getResultList();
        } finally {
            logger.debug("find by CriteriaQuery<T>, elapsedTime={}, nativeSql={}", watch.elapsedTime(), nativeSql);
        }
    }
